...
class PPTXToolUI:
    ...
    def list_files_and_folders(self, folder_selected):
        ...
        tk.Label(header_frame, text="缩略图", width=10).grid(row=0, column=1, padx=5)  # 新增缩略图列
        tk.Label(header_frame, text="原始文件名", width=30).grid(row=0, column=2, padx=5)
        tk.Label(header_frame, text="目标文件名", width=30).grid(row=0, column=3, padx=5)
        ...
        for index, name in enumerate(files_and_folders):
            ...
            if os.path.isdir(original_path) and not self.rename_folders.get():
                continue  # 跳过文件夹
            orig_name = name
            std_name = self.file_operations.generate_standardized_name(original_path, self.add_date.get(), self.rename_folders.get())
            ...
            row_frame.grid(row=index + 1, column=0, sticky='nsew', pady=10)  # 间距设置为20像素

            Checkbutton(row_frame, variable=var, width=10).grid(row=0, column=0, padx=5)
            ...
            thumbnail_label.grid(row=0, column=1, padx=5)

            orig_entry = tk.Entry(row_frame, width=30, textvariable=orig_name_var, state='readonly')
            orig_entry.grid(row=0, column=2, padx=5)
            orig_entry.bind("<Button-1>", lambda e, p=original_path: self.file_operations.open_file(p))
            tk.Entry(row_frame, width=30, textvariable=std_name_var).grid(row=0, column=3, padx=5)
    ...
    def update_buttons(self):
        ...
        if self.redo_history:
            Button(self.button_frame, text="重做", command=self.redo_ename).pack(side=tk.LEFT, pady=10, padx=5)
