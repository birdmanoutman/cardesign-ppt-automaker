...
# 创建并配置按钮，增加间距并左对齐
btn_extract = Button(button_frame, text="Extract Text from PPTX", command=extract_text)
btn_extract.pack(pady=20, anchor='w')  # 增加pady值以增加间距，并使用anchor='w'实现左对齐

btn_adjust = Button(button_frame, text="Adjust Text Boxes", command=adjust_boxes)
btn_adjust.pack(pady=20, anchor='w')  # 同上，保持风格一致

# 添加新按钮以清理母版
btn_clean_master = tk.Button(button_frame, text="Clean Master in PPTX", command=lambda: open_ppt_selection_window('clean_master'))
